-- Show friends the FULL list (watchlist + favorites + watched), not only watched.
-- Previously get_friend_movies / get_friend_tv_shows filtered on is_watched = true,
-- so a friend saw an empty list when the owner's items were only in their
-- watchlist/favorites. Remove that filter while keeping the friendship check
-- and per-title deduplication.
